...
function nn.ProperJacobian.backward(module, input, param, dparam)
   local doparam = 0
   if param then
      doparam = 1
   end
...
function nn.ProperJacobian.backwardUpdate(module, input, param)
...
function nn.ProperJacobian.forward(module, input, param, perturbation)
   param = param or input
   -- perturbation amount
...
function nn.ProperJacobian.backwardDiagHessian(module, input, diagHessianParamName)
...
function nn.ProperJacobian.linearModuleDiagHessian(module, input, gradParamName)
   -- Compute the second derivatives (diagonal Hessian elements)
   ...
   -- F(x_1, x_2, ..., x_n) = y_1^2/2 + y_2^2/2 + ... + y_m^2/2.
   --
   -- (See the the comment for nn.ProperJacobian.backwardDiagHessian() for explanation.)
   --
...
function nn.ProperJacobian.forwardUpdate(module, input, param, perturbation)
   -- perturbation amount
   perturbation = perturbation or 1e-6
   -- 1D view of input
   --local tst = param:storage()
...
function nn.ProperJacobian.testJacobian(module, input, minval, maxval, perturbation)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
   input:copy(torch.rand(input:nElement()):mul(inrange):add(minval))
   local jac_fprop = nn.ProperJacobian.forward(module, input, input, perturbation)
   local jac_bprop = nn.ProperJacobian.backward(module, input)
   local error = jac_fprop-jac_bprop
   return error:abs():max()
end

function nn.ProperJacobian.testJacobianParameters(module, input, param, dparam, minval, maxval, perturbation)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
   input:copy(torch.rand(input:nElement()):mul(inrange):add(minval))
   param:copy(torch.rand(param:nElement()):mul(inrange):add(minval))
   local jac_bprop = nn.ProperJacobian.backward(module, input, param, dparam)
   local jac_fprop = nn.ProperJacobian.forward(module, input, param, perturbation)
   local error = jac_fprop - jac_bprop
   return error:abs():max()
end

function nn.ProperJacobian.testJacobianUpdateParameters(module, input, param, minval, maxval, perturbation)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
   input:copy(torch.rand(input:nElement()):mul(inrange):add(minval))
   param:copy(torch.rand(param:nElement()):mul(inrange):add(minval))
   local params_bprop = nn.ProperJacobian.backwardUpdate(module, input, param)
   local params_fprop = nn.ProperJacobian.forwardUpdate(module, input, param, perturbation)

   local error = params_fprop - params_bprop
   return error:abs():max()
end

function nn.ProperJacobian.testDiagHessian(module, input, gradParamName, diagHessianParamName, minval, maxval)
   ...
   local h_bprop = nn.ProperJacobian.backwardDiagHessian(module, input, diagHessianParamName)
   local h_linearmodule = nn.ProperJacobian.linearModuleDiagHessian(module, input, gradParamName)
   local error = h_bprop - h_linearmodule
   return error:abs():max()
end

function nn.ProperJacobian.testDiagHessianInput(module, input, minval, maxval)
   return nn.ProperJacobian.testDiagHessian(module, input, 'gradInput', 'diagHessianInput', minval, maxval)
end

function nn.ProperJacobian.testDiagHessianWeight(module, input, minval, maxval)
   return nn.ProperJacobian.testDiagHessian(module, input, 'gradWeight', 'diagHessianWeight', minval, maxval)
end

function nn.ProperJacobian.testDiagHessianBias(module, input, minval, maxval)
   return nn.ProperJacobian.testDiagHessian(module, input, 'gradBias', 'diagHessianBias', minval, maxval)
end

function nn.ProperJacobian.testIO(module,input, minval, maxval)
   minval = minval or -2
   maxval = maxval or 2
   local inrange = maxval - minval
...
function nn.ProperJacobian.testAllUpdate(module, input, weight, gradWeight)
   local gradOutput
